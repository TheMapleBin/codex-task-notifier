...
Windows 上的轻量 Codex Desktop/CLI 任务终态通知器。当前生产路径只有一个 Node watcher：读取 Codex rollout JSONL，写入持久 outbox，并调用微信公众号测试号模板消息 API。

```text
Codex rollout JSONL -> watcher -> durable outbox -> WeChat test account
```
...
- 2026-07-28 已将生产 adapter 从直接 iLink 切换到微信公众号测试号；收件人确认个人微信连续收到两条“Codex 通知”。
...
- 直接 iLink DPAPI 配置保留为回滚路径；生产选择器位于仓库外的 DPAPI 配置目录，不需要 OpenClaw、`account/target` 或重复绑定。
...
当前生产首次配置运行 `configure-wechat-test-account.cmd`，在独立终端输入测试号的 AppID、AppSecret、接收人 OpenID 和模板 ID；四项值使用当前 Windows 用户的 DPAPI 保存到仓库外。生产切换使用 `use-wechat-test-account.cmd`，回滚到已保留的直接 iLink 配置使用 `use-ilink.cmd`。
