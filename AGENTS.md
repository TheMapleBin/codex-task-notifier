...
This project is indexed by GitNexus. Use the index instead of blind grepping when you
need callers, callees, or execution flows. Current graph size is reported by the
tool-managed block at the end of this file, which `npx gitnexus analyze` regenerates —
do not hand-maintain those counts here, and do not delete that block.
