...
当前软件验证为 `npm run check` 与 `npm test`，后者为 31/31 通过。只有 [现场验收记录](live-acceptance.md) 中逐层留证的用例才构成阶段 5 证据；自动化测试本身不构成阶段 5 或 6 现场验收。2026-07-28 的 CLI 正常完成验收早于最终输出和一键控制功能，不能据此宣称微信已显示该新增输出段或控制器已经生产启用。
