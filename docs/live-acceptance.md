...
用户报告真实消息尾部包含 `<oai-mem-citation>` 内部元数据且部分输出过早截断。统一净化层现会在 2400 字符截断前清除完整、残缺和转义的内部元数据，并保留安全任务名称与真实 `session_meta` 子代理过滤；41/41 自动化测试通过。修复后的微信显示需要由后续真实完成通知再次确认。
