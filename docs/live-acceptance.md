...
| 用户中断 | 未验收 |
| 微信暂时离线后恢复 | 未验收 |
| ClawBot 保活后跨过期窗口投递 | 未验收；需等待超过既往失效窗口后真实发送并由用户确认 |
...
2026-07-28 实测省略 context 的主动消息虽然 HTTP 200，但用户确认微信未收到；随后发送“绑定”后，两条 pending 自动补发。最新会话状态下 `getconfig`、`typing`、`cancel` 均返回成功，现已将 30 秒保活加入唯一 supervisor。整机冷启动和跨过期窗口后的真实投递仍未验收。
