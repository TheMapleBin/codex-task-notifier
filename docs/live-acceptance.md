...
用户报告真实消息尾部包含 `<oai-mem-citation>` 内部元数据且部分输出过早截断。统一净化层现会在 2400 字符截断前清除完整、残缺和转义的内部元数据，并保留安全任务名称与真实 `session_meta` 子代理过滤；41/41 自动化测试通过。随后一条真实根任务完成通知已由用户确认内容完整，末尾没有其他内部文字，此项现场复验通过。
