...
本文件只记录不含敏感值的现场证据。account、完整 target、context token、实际通知正文、用户 prompt、用户消息、源码和原始请求/响应正文均未写入文档或 Git。
...
范围说明：该验收发生在 `373fd3d` 增加最终 assistant 输出字段和后续一键控制器之前。因此它证明 watcher、outbox、OpenClaw 命令和微信送达链路，但不证明新增“输出”段已经在微信端正确显示，也不证明一键入口已生产启用。当前软件为 31/31 自动化测试通过；后续现场测试必须另行确认净化、截断后的输出段实际收到。
