...
范围说明：该验收发生在 `373fd3d` 增加最终 assistant 输出字段之前。因此它证明 watcher、outbox、OpenClaw 命令和微信送达链路，但不证明新增“输出”段已经在微信端正确显示。该字段目前只有 30/30 自动化测试证据；后续现场测试必须另行确认净化、截断后的输出段实际收到。
