...
当前唯一 lifecycle supervisor 每 30 秒调用一次 `getconfig`，随后执行 `typing/cancel`；真实调用已取得 `getconfig ret=0`、typing `ret=0` 和 cancel `ret=0`。该方案不增加 OpenClaw、Gateway、计划任务或常驻进程，watcher 仍按 Codex 启停。短窗口延长后的真实消息验收仍需完成，不能仅凭心跳返回码宣称长期永不过期。
