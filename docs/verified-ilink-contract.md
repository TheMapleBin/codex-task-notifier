...
当前唯一 lifecycle supervisor 每 30 秒调用一次 `getconfig`，随后执行 `typing/cancel`；真实调用已取得 `getconfig ret=0`、typing `ret=0` 和 cancel `ret=0`。该方案不增加 OpenClaw、Gateway、计划任务或常驻进程，watcher 仍按 Codex 启停。未重新发送“绑定”、连续保活约 4 分钟后，主动发送返回 message ID，用户确认微信实际收到。该证据覆盖此前容易失效的短窗口，但不能外推为腾讯保证长期永不过期。
