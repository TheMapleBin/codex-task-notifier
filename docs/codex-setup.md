...
- iLink 在重新连接后要求用户先向机器人发送一条消息才能恢复事件推送，因此自动模式保持单个轻量 watcher 常驻，不随 Codex Desktop/CLI 退出而停止。
...
日常建议运行一次 `enable-auto-notifier.cmd`，之后无需手工启动。电脑重启、网络重连或 watcher 异常重启后，若状态提示 context 失效，只需在微信里向机器人发一条消息，不需要重新扫码。手工入口仍保留为 `start-notifier.cmd`、`notifier-status.cmd` 和 `stop-notifier.cmd`。
